...
int withinSameBlock(int * ptr1, int * ptr2) {
  // Must be in the same 64-byte aligned block
  int mask = (1 << 31) >> 25; // 0b11111111_11111111_11111111_11000000
  return (int)(((int)ptr1 & mask) == ((int)ptr2 & mask));
}
...
int withinArray(int * intArray, int size, int * ptr) {
  // TODO
  return ((ptr < (intArray + size)) && (ptr >= intArray));
}
/*
...
int invert(int x, int p, int n) {
  // all 1s before n-bit shifted to the left by p bits
  int ones_bn = ((1 << n) - 1) << p;
  // toggle desired bits by XOR-ing x by ones_bn
  return (x ^ ones_bn);
}
