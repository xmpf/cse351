...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: When using numbers greater than 10 (ten), this program
  // overwrites the stack where the array variable is.
  // Although, when using number 11 (elevent) to 14 (fourteen),
  // the program is running without any errors. 
  // This is undefined behavior as the
  // program overwrites memory locations of variable 'value' and 
  // structure 'four_ints'.
  // For 'len' greater than or equal to 15 (fifteen),
  // the program exits with SIGSEGV signal issuing a segmentation fault.
  fillArray(array, 10);
  ...
  // C++ programming.
  // Answer: Without 'free' function, the program
  // fails to free the memory space allocated for
  // 'heap_array', hence a memory leakage of 5 * sizeof(int) bytes
  // occurs.
  free(heap_array);
  ...
  FourInts *structP = (FourInts *)malloc(1 * sizeof(FourInts));
  if (structP == NULL) {
    fprintf(stderr, "Error: Unable to allocate memory!\n");
    return 1;
  }
  
  fillArray((int *)structP, 4);
